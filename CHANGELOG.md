...
- Corrected a visual-review defect in the larger type treatment: labor labels now own their full grid column instead of overlapping the tracks, labor and waste bars are thick enough to scan, and the agent-collaboration line is a distinct capability callout.
...
- Replaced the old underlined text-action treatment with one shared enclosed secondary-button style across the desk, including Reset, Dismiss, removal, download, filter, and navigation actions.
